...
/**
 * @swagger
 * /api/admin/verifications/doctors:
 *   get:
 *     summary: List doctor licence-verification reviews (Admin only)
 ...
 *     responses:
 *       200: { description: Doctor verification reviews }
...
async function getHandler(req: AuthenticatedRequest) {
  ...
  try {
    const result = await new ProviderVerificationsService().listDoctors({
    ...
    return NextResponse.json({ success: false, message: 'Failed to fetch doctor verifications' }, { status: 500 });
  }
}
